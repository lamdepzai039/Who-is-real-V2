...
(() => {
  ...
  socket.on('game:positions', ({ players: nextPlayers }) => {
  ...
  function startMovementLoop() {
    ...
    moveInterval = setInterval(() => {
      if (!SESSION.playerId || !myId()) return;
      const dx = (moveState.KeyD ? 1 : 0) - (moveState.KeyA ? 1 : 0);
      const dy = (moveState.KeyS ? 1 : 0) - (moveState.KeyW ? 1 : 0);
      if (!dx && !dy) return;
      socket.emit('game:move', { dx, dy });
    }, 80);
  }
  ...
  function renderArena() {
    ...
    ctx.fillStyle = 'rgba(73, 215, 224, 0.12)';
    ctx.beginPath();
    ...
    players.forEach((p) => {
      ...
      const color = p.alive ? (isSelf ? '#6ee7f9' : '#4ad7d8') : '#64748b';

      ctx.beginPath();
      ctx.fillStyle = color;
      ctx.arc(px, py, isSelf ? 16 : 13, 0, Math.PI * 2);
      ctx.fill();
