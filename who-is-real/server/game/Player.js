...
class Player {
  constructor({ socketId, name }) {
    ...
    this.isHost = false;
    this.connected = true;
    this.disconnectedAt = null; // set while in the reconnect grace period

    // Secret token proving ownership of this identity across reconnects.
    // NEVER included in toPublic() - only sent to the player themselves.
    this.reconnectToken = randomUUID();

    // Position / movement (server authoritative)
    this.position = { x: 160, y: 180 };
