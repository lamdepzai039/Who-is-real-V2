...
class GameEngine {
  ...
  start() {
    ...
    const { imposters } = assignRoles(players);
    this.room.game = this;
    this.room.state.transition(STATES.STARTING);

    const positions = this._spawnPositions(players.length);
    players.forEach((player, index) => {
      const pos = positions[index] || { x: 180 + (index % 4) * 120, y: 150 + Math.floor(index / 4) * 120 };
      player.position = { x: pos.x, y: pos.y };
      this.emitToPlayer(player, 'game:you', this.privateViewFor(player));
    });
  ...
  movePlayer(player, dx, dy) {
    ...
    if (this.room.state.current === STATES.LOBBY || this.room.state.current === STATES.GAME_OVER) return false;

    const step = 18;
    const nextX = clamp(player.position.x + dx * step, 40, 680);
    const nextY = clamp(player.position.y + dy * step, 40, 380);
  ...
  syncPositions() {
    ...
    this.io.to(this.room.code).emit('game:positions', payload);
  }
  ...
  _spawnPositions(playerCount) {
    const slots = [
      { x: 120, y: 120 }, { x: 600, y: 120 }, { x: 120, y: 300 }, { x: 600, y: 300 },
      { x: 360, y: 90 }, { x: 360, y: 330 }, { x: 200, y: 210 }, { x: 520, y: 210 },
      { x: 120, y: 210 }, { x: 600, y: 210 }, { x: 360, y: 150 }, { x: 360, y: 270 },
    ];
    return Array.from({ length: Math.max(playerCount, 1) }, (_, index) => slots[index % slots.length]);
  }
